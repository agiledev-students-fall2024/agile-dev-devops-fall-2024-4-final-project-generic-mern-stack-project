...
const BlogPost = ({ post, isReply = false }) => {
  ...
  return (
    ...
    >
      <div className="flex flex-row items-center">
        <img
          src={blogPost.user?.profile_pic || "https://via.placeholder.com/150"}
          alt="Profile"
          className="w-20 h-20 rounded-lg my-4 mx-2"
        />
        <p className="flex flex-col justify-start items-start text-md my-4">
          <span className="font-bold text-ebony text-left">
            {blogPost.user?.display_name || "Guest User"}
          </span>
          <span className="text-rose opacity-[75%]">
            @{blogPost.user?.username || "guest"}
          </span>
        </p>
      </div>

      <div className="w-[95%] m-auto text-lg text-ebony">
        {blogPost.content || "This is a placeholder text for a post that doesn’t contain any content."}
      </div>
